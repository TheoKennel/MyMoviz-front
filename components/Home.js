...
function Home() {
  ...
  useEffect(() => {
    fetch('https://my-moviz-back-end-gules.vercel.app/movies')
    .then(response => response.json())
    .then(data => {
      // const dataTab = []
      // const okTab = dataTab.push(data.movies)
      // movieDataApi.push(data.movies);
     setMovieData(data.movies)
    });
  }, []);
  console.log(movieData)

  const movieDataApiOk = movieData.map((data, index) => {
  ...
  // const isMovieLiked = (movieTitle) => {
  //   return likedMovies.some(movie => movie === movieTitle);
  // };
